  ...
  alerts: [
    {
      threat: {
        ...
        severity: "high",
        confidence: 0.91,
        signals: ["300 cases 48hrs", "double normal rate", "Dhaka cluster"],
      },
      verification: {
        credibility_score: 0.88,
        source_tier: "tier1",
        verified: true,
        reason: "Confirmed by Ministry of Health Bangladesh",
      },
      forecast: {
        risk_level: "red",
        escalation_probability_7d: 0.82,
        at_risk_regions: ["West Bengal", "Myanmar"],
        recommended_actions: [
          "Issue travel advisory",
          "Activate mosquito control",
      ...
      summary:
        "A significant dengue fever cluster has emerged in Dhaka, Bangladesh with case counts doubling in 48 hours. Immediate vector control measures are recommended and neighboring regions should heighten surveillance.",
      steps: [
        ...
        { agent: "Policy", status: "done", message: "Recommendations generated" },
        { agent: "Explainability", status: "done", message: "Reasoning chain explained" },
    ...
    },
    {
      threat: {
        ...
        severity: "medium",
        confidence: 0.74,
        signals: ["elevated ILI", "3 provinces", "Java"],
      },
      verification: {
        credibility_score: 0.72,
        source_tier: "tier2",
        verified: true,
        reason: "Reported by regional health authority",
      },
      forecast: {
        risk_level: "orange",
        escalation_probability_7d: 0.55,
        at_risk_regions: ["Malaysia", "Singapore"],
        recommended_actions: [
      ...
      summary:
        "Three provinces in Java, Indonesia are reporting elevated influenza-like illness above seasonal norms. Risk of regional spread to Malaysia and Singapore is moderate over the next 7 days.",
      steps: [
        { agent: "Sentinel", status: "done", message: "Health signals detected" },
    ...
    },
    {
      threat: {
        ...
        severity: "medium",
        confidence: 0.68,
        signals: ["waterborne", "monsoon season", "Bihar"],
      },
      verification: {
        credibility_score: 0.65,
        source_tier: "tier2",
        verified: false,
        reason: "Unconfirmed regional reports",
      },
      forecast: {
        risk_level: "yellow",
        escalation_probability_7d: 0.38,
        at_risk_regions: ["Nepal", "Bangladesh"],
        recommended_actions: [
          "Monitor water quality",
          "Coordinate with WASH teams",
        ],
      },
      final_risk_level: "yellow",
      article: {
        title: "Waterborne illness reports in Bihar following floods",
        source: "Times of India",
        published_at: new Date().toISOString(),
      },
      summary:
        "Unconfirmed reports of waterborne illness in Bihar, India following monsoon flooding. Standard post-flood surveillance protocols recommended.",
      steps: [
        { agent: "Sentinel", status: "done", message: "Health signals detected" },
...
function AgentStep({ step, index }: { step: { agent: string; status: string; message: string }; index: number }) {
...
interface AlertData {
  threat?: { disease?: string; location?: string; severity?: string; confidence?: number; signals?: string[] };
  verification?: { credibility_score?: number; source_tier?: string; verified?: boolean; reason?: string };
  forecast?: { risk_level?: string; escalation_probability_7d?: number; at_risk_regions?: string[]; recommended_actions?: string[] };
...
function AlertCard({ alert, onClick, selected }: { alert: AlertData; onClick: () => void; selected: boolean }) {
...
function CountryRiskMap({ risks }: { risks: Record<string, string> }) {
  const entries = Object.entries(risks);
  if (!entries.length) return <div className="text-center text-slate-500 py-8 text-sm">No risk data yet — run a scan</div>;
  return (
...
export default function BlueBloodDashboard() {
  const [scanData, setScanData] = useState<typeof MOCK_SCAN | null>(null);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-wider text-white">
                BLUEBLOOD<span className="text-cyan-400">.AI</span>
              </h1>
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            {[
              ...
              { label: "Critical Alerts", value: alerts.filter((a) => a.final_risk_level === "red").length, color: "text-red-400" },
              {
                ...
                value: Object.keys(scanData.country_risks ?? {}).filter((k) => (scanData.country_risks as Record<string, string>)[k] !== "green").length,
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-1 space-y-3">
            ...
            {!scanning && alerts.map((alert, i) => (
              <AlertCard key={i} alert={alert} selected={selectedAlert === alert} onClick={() => setSelectedAlert(selectedAlert === alert ? null : alert)} />
            ))}
            ...
            {!selectedAlert ? (
              ...
              <div className={`rounded-xl border p-4 space-y-4 ${selectedRisk?.border} ${selectedRisk?.bg}`}>
                ...
                {selectedAlert.explainability?.why_flagged && (
                  <div>
                    <p className="text-xs text-slate-500 mb-2 tracking-widest">WHY FLAGGED</p>
                    <div className="rounded-lg bg-black/20 p-3">
                      <p className="text-xs text-slate-300">{selectedAlert.explainability.why_flagged}</p>
                      {selectedAlert.explainability.evidence_chain && selectedAlert.explainability.evidence_chain.length > 0 && (
          ...
          <div className="col-span-1 space-y-4">
            ...
            <div className="rounded-xl border border-cyan-500/10 bg-cyan-500/5 p-4">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Just as horseshoe crab blood has protected human health for millions of years, BlueBlood.ai extends that legacy — an AI immune system for global
                public health.
              </p>
              <div className="mt-3 flex flex-wrap gap-1">
                {["Sentinel", "Verification", "Forecast", "Policy", "Explainability", "Coordinator"].map((a) => (
                  <span key={a} className="text-xs px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                    {a}
                  </span>
                ))}
              </div>
              <div className="mt-2 text-xs text-slate-600">
                Powered by Azure AI Foundry · o4-mini · Multi-Agent Reasoning
              </div>
            </div>

            {scanData?.timestamp && (
              <p className="text-xs text-slate-600 text-center">Last scan: {new Date(scanData.timestamp).toLocaleTimeString()}</p>
            )}
          </div>
        </div>
      </div>
